...
export const convertSeconds = (seconds: number): string => {
  ...
  const hourString = hours > 0 ? `${hours} час${hours > 1 ? 'ов' : ''}` : ''
  const minuteString = minutes > 0 ? `${minutes} минут` : ''
  const secondString = remainingSeconds > 0 ? `${remainingSeconds} секунд` : ''

  if (hours > 0) {
    return `${hourString} : ${minuteString || '0 минут'} ${
